...
class App {
  ...
  onTouchDown(e) {
    // Verificar se o evento está dentro do container ou canvas
    const target = e.target || e.touches?.[0]?.target;
    if (!this.container.contains(target) && !this.gl.canvas.contains(target)) return;
    this.isDown = true;
  ...
  onTouchMove(e) {
    ...
    // Durante o drag, permitir movimento mesmo fora do container
  ...
  addEventListeners() {
    ...
    // Resize precisa ser na window
    window.addEventListener('resize', this.boundOnResize);
    // Eventos de início apenas no container/canvas
    this.container.addEventListener('mousewheel', this.boundOnWheel);
    this.container.addEventListener('wheel', this.boundOnWheel);
    this.container.addEventListener('mousedown', this.boundOnTouchDown);
    this.container.addEventListener('touchstart', this.boundOnTouchDown);
    // Eventos de movimento e fim na window para permitir drag mesmo fora do container
    window.addEventListener('mousemove', this.boundOnTouchMove);
    window.addEventListener('mouseup', this.boundOnTouchUp);
    window.addEventListener('touchmove', this.boundOnTouchMove);
    window.addEventListener('touchend', this.boundOnTouchUp);
    // Também adicionar no canvas para garantir
  ...
  destroy() {
    ...
    // Remover listeners do container
    if (this.container) {
